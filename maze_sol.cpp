...
/************************************************************/
/** models a BOARD                                         **/
/** the constructor uses 2 methods from this class to print**/
/** how the maze looks like and it also reads txt file     **/
class Board
{
    private:
            int k;
...
Board()
{
     ...
     file.open("maze007.txt");

     if(file.good()==true)
      {
       file>>lines>>columns;
       if(lines<=0 || columns<=0)
         { cout<<"wrong data wrong data in the txt file!!"<<endl;
             getchar(); exit(-7);}
       ...
       for(int i=0;i<lines;i++)
       {
        for(int j=0;j<columns;j++)
        {
         file>>Tab[i][j].value;
                     ...
                     { cout<<"wrong data wrong data in the txt file!!"<<endl; getchar(); exit(-2);}
        }
       }
...
/**         Search                                         **/
/** There are 2 other methods which print a created tree   **/
         ...
         do
          {
            D.x=-1;M.x=-1;
             ...
             while(M.x<0 || M.x>=B.lines || M.y<0 || M.y>=B.columns){
              ...
              cin>>M.y;}
               if(D.x==M.x && D.y==M.y){
                cout<<"coordinates can not b equal in this game buddy!!"<<endl;}
           }while(D.x==M.x && D.y==M.y);

         if(B.Tab[D.x][D.y].value!=0 || B.Tab[M.x][M.y].value!=0)
         {cout<<"wrong coordinates, u cant start or finish at the wall"<<endl;}
